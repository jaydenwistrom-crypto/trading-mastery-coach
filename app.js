...
function renderToday() {
  ...
  const percent = Math.round((state.completedConcepts.length / 6) * 100);
  ...
  content.querySelector("#due-count").textContent = String(dueNow || Math.max(3 - practicedConcepts, 0));
...
function conceptStatus(conceptId) {
  const stats = state.reviewStats[conceptId] || { correct: 0, attempts: 0 };
  if (stats.correct >= 2 && stats.attempts >= 2) return "MASTERED";
...
function recordAnswer(question, correct) {
  const stats = state.reviewStats[question.concept] || { correct: 0, attempts: 0 };
  stats.attempts += 1;
...
function renderMistakes() {
  ...
  viewRoot.innerHTML = `
    <section class="workspace-panel">
      ...
      <div class="mistake-list">
        ${openMistakes.length ? openMistakes.map((item, index) => `
          <article class="mistake-item">
            <div><span class="status-chip not-shown">NEEDS REVIEW</span><h3>${item.concept}</h3><p>${item.note}</p></div>
            <button class="button button-ghost" type="button" data-resolve="${index}">I can explain this now</button>
          </article>`).join("") : `
  ...
  viewRoot.querySelectorAll("[data-resolve]").forEach((button) => {
    button.addEventListener("click", () => {
      const target = openMistakes[Number(button.dataset.resolve)];
      const original = state.mistakes.find((item) => item.questionId === target.questionId && !item.resolved);
      if (original) original.resolved = true;
      scheduleReview(target.conceptId, "hard");
      saveState();
      renderMistakes();
    });
  });
...
function renderMastery() {
  ...
  viewRoot.querySelectorAll("[data-open-practice]").forEach((button) => button.addEventListener("click", () => setView("practice")));
}
    ...
    <div class="evidence-layout">
      ...
      <aside class="evidence-side">
        <article class="source-card">
          ...
          <a class="button button-ghost" href="https://github.com/jaydenwistrom-crypto/trading-pdf-library/blob/main/futures/focus-cycle/lessons/66586089_FUTURES_FOCUS_PART_I.md" target="_blank" rel="noopener noreferrer">Open evidence file ↗</a>
        </article>
  ...
  register({
    ...
    execute() {
      return {
        ...
        dueReviews: state.reviews.filter((item) => item.due <= Date.now()).length,
      };
    },
  });
